-- Migration: automate points awards for core engagement activities
-- Implements fractional points, centralized logging helper, and activity triggers.

begin;

-- Allow fractional point balances and transaction amounts
...
-- Track transaction activity/source metadata
alter table public.points_transactions
...
-- Helper to wrap profile balance adjustments and transaction logging
...
-- Track social shares (post-level)
...
create policy if not exists "Post shares are viewable by everyone"
  on public.post_shares for select
  using (true);

create policy if not exists "Users can insert their own post shares"
  on public.post_shares for insert
  with check (auth.uid() = user_id);

create policy if not exists "Users can delete their own post shares"
...
-- Track app-level share events (referral, profile, etc.)
...
create policy if not exists "App share events are viewable by creators"
  on public.app_share_events for select
  using (auth.uid() = user_id);

create policy if not exists "Users can insert their own app share events"
  on public.app_share_events for insert
  with check (auth.uid() = user_id);

grant select, insert on public.app_share_events to authenticated;

-- Helper predicate used by multiple triggers
...
-- Post share rewards and counters
